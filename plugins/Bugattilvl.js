...
`.trim()
    ...
    try {
      let imgg = `https://api.shannmoderz.xyz/server/file/XyjKP6IA0VnyFZF.jpg${encodeURIComponent(name)}&currxp=${user.exp - min}&needxp=${xp}&level=${user.level}&rank=${encodeURIComponent(pp)}&avatar=${encodeURIComponent(pp)}&background=${encodeURIComponent(background)}`
...
`.trim()
    ...
    try {
      let img = `https://api.shannmoderz.xyz/server/file/XyjKP6IA0VnyFZF.jpg${encodeURIComponent(pp)}`
